...
const TrainingPlacePicker = () => {
  const { trainingPlaces } = useSelector((state: RootState) => state.survey.trainingInfo);
  const dispatch = useDispatch<AppDispatch>();
  const [selected, setSelected] = useState(trainingPlaces);

  return (
    <QuestionRenderer
      title="Де плануєш тренуватись?"
      redirectPath={SURVEY_TRAINING_ROUTES.EQUIPMENT}
    >
      <div className="text-center mt-10 flex flex-col gap-5">
        {USER_CONSTANTS.TRAINING_PLACES.map((opt) => (
          <CustomCheckbox
            key={opt}
            label={opt}
            ...
            onChange={() => {
              setSelected(opt);
              dispatch(setTrainingInfoItem({ key: 'trainingPlaces', value: opt }));
            }}
            type="checkbox"
...
export default TrainingPlacePicker;
